package week06.sandy.id.ac.umn;
...
public class Tugas1 {
	...
	public static class Order {
		private int id;
		private Handphone handphone; // ga dipake disini
		private Voucher voucher; // ga dipake disini
									// karena apa? karena kalo pake array list bisa improve fitur tambah order lain
									// sekaligus (kalo niat)
...
//			});
		...
		public String toString() {
			return "Order{" + "ID=" + id + ", Barang= " + getBarangList() + ", Tipe Barang= " + getBarangList()
					...
					+ ", Total Quantity Barang= " + jumlahBarang + ", "
		...
		public void orderDisplay() {
			...
			System.out.println("PAYMENT TOTAL\t" + RED_TEXT + "Rp" + rupiahFormat.format(uangBayaran) + RESET_COLOR
					+ "\nRETURN CHANGE\t" + GREEN_TEXT + "Rp"
					+ rupiahFormat.format((uangBayaran - totalHargaSetelahTax)) + RESET_COLOR);
			System.out.println("-----------------------------------------------\n");
		}
	}
	...
	public static class Barang {
		...
		public Barang(int id, int stok, double harga, String nama, boolean isStokEmpty, boolean isTempStokEmpty,
				int tempStok) {
			this.id = id;
			this.stok = stok;
			this.harga = harga;
...
//			System.out.println("tes");
	...
	public static class Handphone extends Barang {
	...
	public static class Voucher extends Barang {
	...
	public static class Main {
		...
		public static void initData() {
			addItemToList(new Handphone(1, "Oppo", 1499999, 10, "Black"));
			addItemToList(new Voucher(2, "Netflix 'n Chill", 1599999, 5, 1.2));
			addItemToList(new Voucher(3, "Google Play", 99999, 100, 0.6));
			addItemToList(new Handphone(4, "Samsung", 1999999, 15, "Blue"));
			addItemToList(new Handphone(5, "iPhone", 2999999, 50, "Yellow"));
			addItemToList(new Voucher(6, "Amazon Gift Card", 499999, 50, 0.8));
			addItemToList(new Voucher(7, "Steam Wallet", 299999, 75, 0.9));
			addItemToList(new Handphone(8, "Xiaomi", 1799999, 30, "White"));
			addItemToList(new Voucher(9, "Spotify Premium", 129999, 120, 0.5));
			addItemToList(new Handphone(10, "Realme", 1599999, 20, "Green"));
			addItemToList(new Voucher(11, "Disney+ Subscription", 999999, 30, 1.0));
			addItemToList(new Voucher(22, "Disney+ Subscription", 999999, 30, 1.0));
			addItemToList(new Voucher(23, "Disney+ Subscription 2", 999999, 30, 1.0));
			addItemToList(new Handphone(12, "OnePlus", 2499999, 25, "Red"));
			addItemToList(new Handphone(13, "Sony Xperia", 2699999, 12, "Purple"));
			addItemToList(new Handphone(14, "Google Pixel", 3299999, 8, "Silver"));
			addItemToList(new Voucher(15, "PlayStation Plus", 799999, 40, 0.7));
			addItemToList(new Handphone(16, "Huawei", 9999999.376, 18, "Gold"));
			addItemToList(new Handphone(17, "Asus ROG", 3999999, 5, "Black"));
			addItemToList(new Handphone(18, "Nokia", 1299999, 22, "Cyan"));
			addItemToList(new Handphone(20, "iPhone 25 Pro Max", 199999999999999.3768, 18, "Titanium Blue"));
			addItemToList(new Handphone(21, "Huawei", 123838383.0000, 18, "Black"));
...
//													: (voucherList.size() > 0 ? voucherList.size() : 1)) / pageSize);
									...
									Order currOrder = new Order(orderHistory.size() + 1);
									...
									try {
										...
										do {
											...
											if (itemID < 0) {
												System.out.println(RED_TEXT + WHITE_BACKGROUND
														+ "Please choose a valid " + itemType + "!" + RESET_COLOR);
												isOrderPaymentValid = false;
												break;
											}

											if (handphoneList.stream().noneMatch(id -> id.getId() == itemID)
...
//										System.out.println(handphoneList.toString());
											...
											selectedItemQuery = tempList.stream().filter(i -> i.getId() == itemID)
													.findFirst();
