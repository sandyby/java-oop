...
public class SandyBonfilioYuvens_Main_CL {
	...
	public static void showAddUserMenu() {
		...
		try {
			...
			Optional<SandyBonfilioYuvens_User_CL> noRekeningQuery = Optional.empty();
			do {
				try {
					isInputValid = true;
					System.out.print("Enter Your Account Number: ");
					long noRekening = scanner.nextLong();
					scanner.nextLine();
					...
					noRekeningQuery = users.stream().filter(u -> (u.getNoRekening() == noRekening)).findFirst();
					isInputValid = false;
					System.out.println(WHITE_BACKGROUND + RED_TEXT
...
//				cellLength = (tempUser.getNama().length() < 50 ? 50 : tempUser.getNama().length());
	...
	public static void handleDeposit() {
		boolean isTransactionInputValid;
	...
	public static void handleTransfer() {
		...
		do {
			...
			try {
				...
				selectedUserQuery = users.stream().filter(u -> (u.getNoRekening() == toRekening)).findFirst();
...
//					System.out.println(WHITE_BACKGROUND + RED_TEXT + ime + RESET_COLOR);
			...
			if (!currentUser.transfer(nominal, toRekening)) {
				System.out.println(WHITE_BACKGROUND + RED_TEXT
						+ "Something went wrong when trying to transfer your money!\nPlease try again later"
						+ RESET_COLOR);
				isTransactionValid = false;
				break;
			}
...
//										.filter(u -> (u.getNama().toLowerCase().equals(selectedUserCleaned))));
								selectedUserQuery = users.stream()
										.filter(u -> (u.getNama().toLowerCase() == selectedUser.toLowerCase()))
										.findFirst();
//								System.out.println(selectedUserQuery);
								System.out.println(selectedUserQuery);
